<?php 

define('ASSETS_VERSION', '1.0.0.0');

require_once 'functions/helper-functions.php';

require_once 'functions/theme-setup.php';

require_once 'functions/include-scripts.php';

require_once 'functions/bs4navwalker.php';

require_once 'functions/enable-svg.php';

require_once 'functions/add-options-page.php';

require_once 'functions/rename-default-post-type.php';

// require_once 'functions/admin-login-style.php';

require_once 'functions/admin-favicon.php';

// require_once 'functions/admin-style.php';

require_once 'functions/remove-comments.php';

require_once 'functions/remove-tax.php';